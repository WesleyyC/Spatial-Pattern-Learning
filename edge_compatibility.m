function [c] = edge_compatibility_spmdl(edge, mdl_edge)
    % node_compatibility function is used to calculate the similarity
    % between node1 and node2
    
    c=0;
    
    if ~edge.hasAtrs()||~mdl_edge.hasAtrs()
    ...
    elseif edge.numberOfAtrs() ~= mdl_edge.numberOfAtrs()    
        ...
        % calculate the score
        c=exp(-(edge_atrs-mdl_edge_atrs)/mdl_edge_cov*(edge_atrs-mdl_edge_atrs))/...
            ((2*pi)^(num_atrs/2)*sqrt(abs(mdl_edge_cov)));
    end
end
